...
export function checkEligibility(issue: WorkItem, state: OrchestratorState, config: Config): string | null {
  ...
  if (state.retryQueue.some((r) => r.workItem.id === issue.id)) return "in_retry_queue";
  if (isInCooldown(state, issue.id, COMPLETION_COOLDOWN_MS)) return "in_cooldown";
  if (state.running.size >= getEffectiveConcurrency(config)) return "concurrency_limit";
  return null;
}

function sortCandidates(issues: WorkItem[]): WorkItem[] {
  return [...issues].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
}

function availableSlots(state: OrchestratorState, config: Config): number {
  return Math.max(0, getEffectiveConcurrency(config) - state.running.size);
}

export function selectForDispatch(candidates: WorkItem[], state: OrchestratorState, config: Config): WorkItem[] {
  const eligible = candidates.filter((issue) => checkEligibility(issue, state, config) === null);
  const sorted = sortCandidates(eligible);
  return sorted.slice(0, availableSlots(state, config));
}

export function selectPending(candidates: WorkItem[], state: OrchestratorState, config: Config): WorkItem[] {
